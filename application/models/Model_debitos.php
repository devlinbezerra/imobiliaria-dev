...
class Model_debitos extends MY_Model {
    ...
    public function report($where = 'd.id > 0'){
        $this->db->select('d.id, pe.nome as inquelino,
        i.descricao as imovel,
        d.vencimento,
        d.valor as valorNominal,
        datediff(date(now()),d.vencimento) as aging,
        pe.telefone_comercial as phone,
        d.status');
        $this->db->from($this->table.' as d');
        $this->db->join('contratos c','d.contrato = c.id');
        $this->db->join('imoveis i','c.imovel = i.id');
        $this->db->join('pessoa pe','c.contratante = pe.id');
        $this->db->join('pessoa po','c.contratado = po.id');
        $this->db->where($where);
        $res = $this->db->get()->result();
        if(sizeof($res) > 0){
            $data = array (
